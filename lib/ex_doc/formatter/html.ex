defmodule ExDoc.Formatter.HTML do
  ...
  def render_all(project_nodes, ext, config, opts) do
    project_nodes
    |> Enum.map(fn node ->
      # TODO: keep app in config? remove app?
      #       app is used to see if currently parsed module is in the same app,
      #       to e.g. generate local vs external URL.
      app =
        case :application.get_application(node.module) do
          {:ok, app} -> app
          :undefined -> nil
        end
  ...
  defp render_doc(%{doc: doc, source_path: file, doc_line: line} = node, autolink_opts, opts) do
    opts = [file: file, line: line + 1] ++ opts
